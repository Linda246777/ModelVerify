...
class IMUDataReader:
    ...
    def check_imu_table(self) -> bool:
        """检查是否存在imu_data表"""
        return self.check_table("imu_data")
...
class CamDataReader:
    ...
    def _select_cam_table(self) -> Optional[str]:
        """选择可用的相机位姿表"""
        if self.check_table("poses"):
            return "poses"
        if self.check_table("track_result"):
            return "track_result"
        return None

    def read_cam_data(self) -> pd.DataFrame:
        """从数据库读取相机位姿数据"""
        if not self.conn:
            raise sqlite3.Error("数据库未连接")

        table_name = self._select_cam_table()
        if not table_name:
            raise sqlite3.Error("数据库中不存在 poses 或 track_result 表")
        ...
        if table_name == "poses":
            ...
            SELECT
                ...
                q_RS_w, q_RS_x, q_RS_y, q_RS_z,
                p_cs_c_x, p_cs_c_y, p_cs_c_z,
                q_cs_w, q_cs_x, q_cs_y, q_cs_z,
                t_system_ns
            FROM poses
            ORDER BY timestamp_ns
            """
        ...
        try:
            ...
            if table_name == "poses":
                ...
                df["t_system"] = df["t_system"] / 1000

            print(f"成功读取 {len(df)} 条相机位姿记录")
            return df

        except sqlite3.Error as e:
            print(f"读取相机位姿数据失败: {e}")
            raise
    ...
    def save_to_csv(self, df: pd.DataFrame, output_path: str):
        ...
        try:
            header_line = (
                ...
                "q_RS_w [],q_RS_x [],q_RS_y [],q_RS_z [],"
                "p_CS_C_x [m],p_CS_C_y [m],p_CS_C_z [m],"
                "q_CS_w [],q_CS_x [],q_CS_y [],q_CS_z [],t_system [us]"
            )
...
def _is_db_file(file_name: str) -> bool:
    return file_name.endswith(".db") and not file_name.endswith(".db-journal")
...
def select_db_file(unit_dir: str) -> Optional[str]:
    """只允许目录同名 .db"""
    base_name = os.path.basename(unit_dir)
    preferred_name = f"{base_name}.db"
    preferred_path = os.path.join(unit_dir, preferred_name)
...
def process_unit_directory(unit_dir: str):
    ...
    db_path = select_db_file(unit_dir)
    if not db_path or not os.path.exists(db_path):
        print(f"数据单元 {unit_dir} 中没有找到数据库文件")
        return

    print(f"处理数据单元: {unit_dir}")
    process_single_database(db_path)
...
def process_single_database(
    db_path: str,
    imu_output_path: Optional[str] = None,
    ...
    if imu_output_path is None:
        imu_output_path = os.path.join(os.path.dirname(db_path), "imu.csv")
    if cam_output_path is None:
        cam_output_path = os.path.join(os.path.dirname(db_path), "cam.csv")
    
    try:
        with IMUDataReader(db_path) as reader:
            if reader.check_imu_table():
                df = reader.read_imu_data()
                reader.save_to_csv(df, imu_output_path)
            else:
                print(f"数据库 {db_path} 中没有 imu_data 表")

        with CamDataReader(db_path) as cam_reader:
            try:
                cam_df = cam_reader.read_cam_data()
                cam_reader.save_to_csv(cam_df, cam_output_path)
            except sqlite3.Error as e:
                print(f"数据库 {db_path} 中没有相机位姿数据: {e}")
...
def main():
    ...
    parser.add_argument("--output", type=str, help="指定IMU输出CSV文件路径")
    parser.add_argument("--imu-output", type=str, help="指定IMU输出CSV文件路径")
    parser.add_argument("--cam-output", type=str, help="指定相机输出CSV文件路径")
    ...
    parser.add_argument("-u", "--unit", type=str, help="指定单个数据单元目录（只读取目录同名 .db）")
    parser.add_argument("-d", "--dir", type=str, help="指定批处理的上级目录，将处理每个包含同名 .db 的子目录作为一个数据单元")
    
    args = parser.parse_args()
    
    if args.db:
        # 处理单个数据库
        imu_output = args.imu_output or args.output
        process_single_database(args.db, imu_output, args.cam_output)
    elif args.unit:
        # 处理单个数据单元
        ...
        print("  --imu-output/--cam-output 参数指定输出文件")
